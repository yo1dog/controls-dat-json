...
function createNMeta(oMeta) {
  var nMeta = {
    ...
    origJSONGeneratedBy: oMeta.xmlGeneratedBy
  };
...
function createNGame(oGame) {
  ...
  try {
    ...
    if (knownIssueStr) {
      console.error('NOTE: There is a known issue with "' + oGame.romname + '" in controls.dat that most likely caused this error: ' + knownIssueStr);
    }
  ...
  var nGame = models.game.create({
    name                   : oGame.romname,     // match MAME nomenclature
    description            : oGame.gamename,    // match MAME nomenclature
    numPlayers             : oGame.numPlayers,
    alternatesTurns        : oGame.alternating, // clearer property name
    hasIdenticalControlSets: oGame.mirrored,    // clearer property name
    usesServiceButtons     : oGame.usesService, // clearer property name
    usesTilt               : oGame.tilt,        // clearer property name
    hasCocktailDipswitch   : oGame.cocktail,    // clearer property name
    notes                  : oGame.miscDetails, // clearer property name
    errors                 : errors,
...
function createControlSetsFromMirroredOGame(oGame) {
  ...
  if (oGame.alternate) {
    ...
    // ex: [1, 2, 3, 4]
    var allPlayerNums = [];
    for (var i = 1; i < oGame.numPlayers; ++i) {
...
function createNControlsFromOControl(oControl, playerNum, mameInputPortToLabelMap) {
  ...
  // check if this game is a special case that requires overriding
  // the player number (see setupSpecialCases function)
...
function createNControlsFromODualControl(controlDef, playerNum, controlButtonMAMEInputPorts, mameInputPortToLabelMap) {
...
function createOutputToInputMapForControl(controlDef, playerNum, controlButtonMAMEInputPorts, mameInputPortToLabelMap, isDual, isLeftDual) {
  ...
  for (var outputKey in controlDef.outputs) {
    ...
    if (defaultMAMEInputPortSuffix === 'BUTTON') {
      ...
      mameInputPort = controlButtonMAMEInputPorts.shift(0);
